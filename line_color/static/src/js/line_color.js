...
const DLC_PATCH_NAME = "line_color.ListRenderer";
...
function getDocumentCacheKey(record, listResModel, ctx) {
    ...
    const linkedDocumentId =
        ...
        ctx.active_id ||
        "unknown";
    const modelKey = rootModel || ctx.active_model || listResModel || "unknown";
    return `${modelKey}:${linkedDocumentId}`;
}

function getOrInitGroupIndexForProduct(record, listResModel, ctx, duplicatePids) {
    const documentKey = getDocumentCacheKey(record, listResModel, ctx);
...
patch(ListRenderer.prototype, DLC_PATCH_NAME, {         // OWL component -> rendering list/tree views
    setup() {
    ...
    _dlcGetContext(record) {
        return record.model?.config?.context || this.props.list?.model?.config?.context || {};
    },

    _dlcGetRecordModelName(record, ctx) {
        return (
            record.resModel ||
            record.model?.root?.resModel ||
    ...
    _dlcIsRecordInEnabledScope(record) {
        const ctx = this._dlcGetContext(record);
        const recordModel = this._dlcGetRecordModelName(record, ctx);

        if (["stock.move", "stock.move.line"].includes(recordModel)) {
        ...
        if (recordModel === "sale.order.line") {
            return this.dlcScope.apply_sale;
        }

        if (recordModel === "purchase.order.line") {
            return this.dlcScope.apply_purchase;
        }

        if (recordModel === "account.move.line") {
    ...
    getRowClass(record) {                               //  compute CSS classes for each row
        const cls = this._super(...arguments) || "";

        if (!this._dlcIsRecordInEnabledScope(record)) return cls;

        const ctx = this._dlcGetContext(record);
        const listResModel = this.props.list?.resModel || "";
        ...
        const groupIndexByProduct = getOrInitGroupIndexForProduct(record, listResModel, ctx, duplicates);
